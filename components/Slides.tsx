...
const Slides: React.FC<SlidesProps> = ({ slides, inputs, chartData }) => {
  ...
  const handleDownload = async () => {
    ...
    for (let i = 0; i < slideElements.length; i++) {
      ...
      // Hide notes for screenshot
      if (notesElement) {
        notesElement.style.display = 'none';
      }

      // Temporarily set a fixed width to ensure consistency for html2canvas
      slide.style.width = '600px';
      const canvas = await html2canvas(slide, { scale: 2 });
      slide.style.width = ''; // Reset style
      ...
      let yPos = (pdfHeight - imgHeight) / 2; // Center image vertically
      if (yPos < 0) yPos = 0;
