  ...
  return (
    ...
    >
      <div id="printable-worksheet-part-1">
        <h1 className="text-center mb-8 font-bold print-item" style={{ fontSize: '24pt' }}>{worksheet.title}</h1>
        <div className="flex justify-between mb-8 pb-4 border-b-2 border-slate-200 print-item">
        ...
        <div className="space-y-8">
            {generalSections.map((section) => (
                <section key={section.title} className="border-b-2 border-slate-200 pb-4 last:border-b-0 print-item-group">
                    <h3 className="font-semibold text-sky-800 mb-3 print-item" style={{ fontSize: '16pt' }}>{section.title}</h3>
                    {section.content && <div className="print-item"><LatexRenderer content={section.content} className="mb-4 italic text-slate-600" enableMarkdown={true} /></div>}
      ...
      {(sources.length > 0) && (
          <div id="printable-worksheet-part-2">
            <div className="my-8 border-2 border-slate-300 bg-slate-50 rounded-lg p-6 print-item">
  ...
  return (
    ...
    >
      <div id="printable-memo-part-1">
        <h1 className="text-center mb-10 font-bold print-item" style={{ fontSize: '24pt' }}>
          {worksheet.title} - Answer Memo
        </h1>
        
        <div className="space-y-8">
          {(worksheet.sections || []).map((section, sectionIndex) => (
            <section key={`memo-${section.title}`} className="border-b-2 border-slate-200 pb-4 last:border-b-0 print-item-group">
              <h3 className="font-semibold text-sky-800 mb-3 print-item" style={{ fontSize: '16pt' }}>
                {section.title}
              </h3>
              <div className="divide-y divide-slate-200">
                {(section.questions || []).map((q, qIndex) => (
                  <div key={`memo-q-${sectionIndex}-${qIndex}`} className="py-3 flex items-start print-item" style={{ breakInside: 'avoid' }}>
  ...
  const handleDownload = async (type: 'worksheet' | 'memo') => {
    ...
    try {
        ...
        await downloadPdf({
            ...
            element: container,
            orientation: 'p',
            margin: 40 // Apply margin in the utility
        });
